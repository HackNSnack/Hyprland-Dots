...
- Custom keybinds (Vivaldi, vim-style hjkl navigation, Matrix lock screen)
- Font size preferences in kitty and rofi
- Matrix video lock screen (MatrixLockVideo.sh)

## Upstream History

- **Original**: JaKooLit/Hyprland-Dots (unmaintained)
- **Current**: LinuxBeginnings/Hyprland-Dots (active)
- The upstream moved from JaKooLit to LinuxBeginnings when JaKooLit stopped maintaining.
- The two-tier config system (configs/ + UserConfigs/) remained the same.
...
4. **Lua removal**: New upstream removed lua/ directory entirely. Accept deletions. Update any references.
